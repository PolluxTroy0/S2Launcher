...
namespace S2Lobby
{
    public class NetworkProcessor
    {
        ...
        private void HandlePacket()
        {
            ...
            {
                Program.Log($"{DateTime.Now} : KO : Incorrect packet magic, is {Message.Magic:X08} but should be {MessageContainer.MagicNumber:X08}");
            }
            if (Message.To != MessageContainer.ConnectionServer)
            {
                Program.Log($"{DateTime.Now} : KO : Incorrect receiver id, is {Message.To:X08} but should be {MessageContainer.ConnectionServer:X08}");
            }

            uint crc32 = Crc32.Calculate(Message.Payload);
            if (crc32 != Message.PayloadChecksum)
            {
                Program.Log($"{DateTime.Now} : KO : Incorrect checksum, is {Message.PayloadChecksum:X08} but should be {crc32:X08}");
            }
        ...
        private void HandlePayload()
        {
            if (Message.Type == MessageContainer.Types.HandshakeConnect)
            {
                ...
                {
                    Program.Log($"{DateTime.Now} : KO : Incorrect handshake connect message size, is {Message.Payload.Length} but should be {HandshakeMessage.MessageSize}");
                }
                if (Message.From != MessageContainer.ConnectionUnknown)
                {
                    Program.Log($"{DateTime.Now} : KO : Incorrect sender id, is {Message.From:X08} but should be {MessageContainer.ConnectionUnknown:X08}");
                }
                ...
                {
                    Program.Log($"{DateTime.Now} : KO : Incorrect handshake magic, is {handshake.Magic:X08} but should be {HandshakeMessage.MagicNumber:X08}");
                }
            ...
            {
                Program.Log($"{DateTime.Now} : KO : Unknown prefix message type: {Message.Type}");
            }
        }
        ...
        {
            Program.Log("{DateTime.Now} : KO : Unknown payload message received");
        }
