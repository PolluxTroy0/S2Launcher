    ...
    public class ServerProcessor : NetworkProcessor
    {
        ...
        protected sealed override void HandleMessage(BinaryReader reader, BinaryWriter writer)
        {
            ...
            if (prefix.Magic == PayloadPrefix.PayloadMagic)
            {
                ...
                {
                    Program.Log($"{DateTime.Now} : KO : Corrupt payload type, first is {prefix.Type1:X04} but second is {prefix.Type2:X04}");
                }
            ...
            else if (prefix.Magic == ChatPayloadPrefix.PayloadMagic)
            {
                if (prefix.Type1 != 0)
                {
                    Program.Log($"{DateTime.Now} : KO : Corrupt payload chatTypes, is {prefix.Type1:X04} but expected 0");
                }
            ...
            {
                Program.Log($"{DateTime.Now} : KO : Incorrect payload magic, is {prefix.Magic:X04} but should be {PayloadPrefix.PayloadMagic:X04}");
            }
        ...
        private void HandleRegisterUser(RegisterUser payload, PayloadWriter writer)
        {
            ...
            Program.Log($"{DateTime.Now} : OK : Account created for {name}");
        }
        ...
        private void HandleLoginUser(LoginUser payload, PayloadWriter writer)
        {
            ...
            Program.Log($"{DateTime.Now} : OK : User {name} logged in");
        }
        ...
        private void HandleLoginServer(LoginServer payload, PayloadWriter writer)
        {
            ...
            Program.Log($"{DateTime.Now} : OK : Server logged in for user {name}");
        }
    }
}
